# 10 cách dựng (Construction Styles) — CÔNG NGHỆ SỐ
...
Cùng bộ 10 slot với tuyến kinh doanh (Apps Script `STYLE_ROTATION` cấp qua `claim_style` dưới
LockService), nhưng ẩn dụ được **diễn giải lại cho tin công nghệ / AI**.
...
| 1 | `2-chip-and-leaderboard` | Chip số liệu viền xanh + bảng xếp hạng model/hãng, watermark số khổng lồ |
...
| 4 | `5-map-and-geo` | Bản đồ — địa chính trị công nghệ (kiểm soát xuất khẩu chip, data center, thị trường) |
...
- **Data moment**: con số chính TO ở giữa, bên dưới là 2–3 cột dọc ngắn so sánh (vd điểm benchmark
  của Astra vs Sol vs Fable) — cột count-up cao dần, số chính là cột cao nhất được tô xanh.
...
- **What happened**: ảnh + panel, thêm 2 chip số liệu viền xanh (vd "3/9/2026" · "trong 1 tuần").
...
- **Data moment**: bảng xếp hạng ngang 3–5 hàng (model / hãng bên trái, thanh + số bên phải), hàng
  của chủ thể tin tô xanh, thanh chạy count-up.
- **Context**: 2–3 chip lớn xếp dọc, mỗi chip 1 dữ kiện nền, viền xanh, số nổi bật.
- **Impact**: 2 chip tác động lớn, watermark số mờ tiếp tục phía sau để liền mạch với act trước.

## Style 2 — Ticker Tape (release feed)

- **What happened**: ảnh nền mờ + 1 dải ngang viền trên/dưới xanh chạy full-width chứa nguồn + tiêu
...
Dùng khi tin có yếu tố địa lý / địa chính trị công nghệ rõ (kiểm soát xuất khẩu chip, data center
...
- **Context**: bản đồ với 3–5 vùng/nước highlight tuần tự (tô xanh đậm dần theo thứ hạng), đường nối
...
- **What happened**: chuẩn ảnh + panel, badge nguồn đổi thành vòng tròn nhỏ viền xanh bao icon nguồn.
...
- **Context**: biểu đồ cột đầy đủ 4–6 cột (xanh / cam), mỗi cột có nhãn số phía trên.
- **Impact**: 2 sparkline ngang xếp chồng, đường vẽ xong thì số cuối dòng mới hiện.

## Nguyên tắc chung cho mọi style (bắt buộc)

- Tuân thủ TOÀN BỘ `BRAND-SYSTEM.md`: màu `#4C8DFF` / `#0B0E14` / `#FF8A5B`, Montserrat mọi vai trò,
...
- Mỗi style là **ẩn dụ hình ảnh**, KHÔNG phải màu mới — Stock Terminal / Ticker KHÔNG dùng xanh lá
  cho "tăng", vẫn dùng `#4C8DFF` cho điểm nhấn tích cực, `#FF8A5B` cho rủi ro / mặt "mối lo".
