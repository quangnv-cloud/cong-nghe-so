# CÔNG NGHỆ SỐ — Hệ thống thương hiệu & sản xuất video tin công nghệ / AI

Nguồn tham chiếu chung cho MỌI video tin công nghệ / AI của kênh. Không lặp lại toàn văn trong từng
`BRIEF.md`, chỉ trích phần liên quan. Đọc song song với `PRODUCTION-WORKFLOW.md` (quy trình thao
tác) và `CONSTRUCTION-STYLES.md` (10 cách dựng xoay vòng).

Tuyến này **tách hoàn toàn** khỏi "BOT BÁN HÀNG · KINH DOANH" (Gmail, Apps Script, repo, token
riêng) nhưng **kế thừa toàn bộ ngữ pháp motion-graphics** của tuyến đó — chỉ đổi màu, chủ đề, và
thêm một act CTA cuối.

## Định vị & tính cách thương hiệu

Bản tin công nghệ hiện đại, gọn, dữ-liệu-hoá, tỉnh táo. Đưa tin + đặt câu hỏi để khán giả tranh
...
| Vai trò | Hex |
| --- | --- |
| Primary — xanh điện | `#4C8DFF` |
| Nền gần đen | `#0B0E14` |
| Card tối | `#121724` |
| Chữ trắng | `#FFFFFF` |
| Cảnh báo / tương phản — cam | `#FF8A5B` |

Không dùng: xanh lá, tím, neon, rainbow gradient. Xanh `#4C8DFF` là màu nhận diện; cam `#FF8A5B`
CHỈ cho tín hiệu cảnh báo / mặt "mối lo" / gạch xoá / nhấn tương phản, không rải khắp khung.
...
- **Keyword highlight**: trong 1 câu, chỉ số liệu / tên riêng nổi bật bằng `#4C8DFF` (hoặc `#FF8A5B`
  nếu là điểm tiêu cực / rủi ro) — không để cả câu cùng visual weight.
- Màu chữ: trắng trên nền tối / ảnh tối; xanh để highlight; cam để highlight điểm rủi ro; chữ tối
...
| Act | Nội dung |
| --- | --- |
| 1. Hook | Masthead (logo + "Công Nghệ Số") + badge nguồn + ngày + tên chủ thể TO + **2 tag tương phản ngắn** (vd "● Mạnh nhất  ● Tranh cãi nhất"). Hiểu được ngay khi tắt tiếng. |
...
| 7. CTA | Đặt lại câu hỏi tranh luận của tin ("bước tiến hay mối lo?") + 2 lựa chọn đối lập (icon CSS: mũi tên lên xanh / tam giác cảnh báo cam) + pill **"Bình luận quan điểm của bạn"** + chữ ký logo. ~5–7s. |
...
**[BẮT BUỘC] KHÔNG đọc tên kênh trong lời thoại** — TTS đọc "Công Nghệ Số" → "Cổng Nghệ Số". Tên
...
- Công thức đầy đủ: `videos/astra-openai/AUDIO-NOTES.md`.

## SFX

Đặt cue tại điểm pop-in text/card quan trọng + điểm chuyển cảnh chính (không phải MỌI lần chuyển
cảnh). Palette tham chiếu: `videos/astra-openai/assets/sfx/` (chime / click-soft / impact-bass /
...
**[Thêm 2026-09-16]** Đúc kết từ video mẫu Tin Tức Số `mau-tin-tuc-so-nang-cap` (phiên 2026-09-15/16,
đạt ~95/100 theo chuẩn quốc tế) — trước đây các kỹ thuật này chỉ tồn tại trong 1 video mẫu dựng
tay, CHƯA từng ghi thành quy tắc nên video routine hàng ngày không áp dụng, vẫn trông "phong cách
cũ" (nền phẳng, không glow, không caption, trống đen nửa dưới khung — xác nhận qua frame thật ngày
16/9 của video `ai-tuyen-dung-han-quoc`). Từ giờ BẮT BUỘC, không phải tùy chọn.
...
<div id="bg-depth" style="position:absolute;inset:0;overflow:hidden;z-index:0;">
  <div class="blob" style="position:absolute;width:640px;height:640px;border-radius:50%;
    background:radial-gradient(circle, rgba(76,141,255,0.20), transparent 70%);
  ...
  <div class="blob" style="position:absolute;width:560px;height:560px;border-radius:50%;
    background:radial-gradient(circle, rgba(255,138,91,0.14), transparent 70%);
...
.evidence-card, .article-image-wrap {
  box-shadow: 0 0 40px rgba(76, 141, 255, 0.16);
}
```
...
4. Mỗi từ là 1 `<span class="w">`, mặc định trắng/mờ, tween sang màu brand (`#4C8DFF`) ĐÚNG lúc từ
...
.cutflash { position: absolute; inset: 0; z-index: 6; pointer-events: none; opacity: 0;
  background: linear-gradient(180deg, rgba(76,141,255,0.4), rgba(255,138,91,0.18)); }
```
```js
...
Cảm giác cần đạt: "một bản tin công nghệ hiện đại thiết kế bằng ngôn ngữ motion graphics" — không
phải "một quảng cáo sản phẩm công nghệ làm thành video".

Video mẫu tham chiếu đầy đủ: `videos/astra-openai/` (OpenAI ra mắt Astra, dịch từ TechCrunch).
