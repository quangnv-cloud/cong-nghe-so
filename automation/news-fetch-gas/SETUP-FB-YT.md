# Setup Apps Script — đăng tự động Facebook + YouTube cho "Công Nghệ Số"

Code đã viết xong ở `automation/news-fetch-gas/Code.gs` (2163 dòng — dùng lại toàn bộ engine
đã chạy production ở kênh "Kinh Tế Số": có sẵn `publish_facebook`, `publish_facebook_photo`,
`publish_youtube`; IG/Threads cũng có sẵn trong code, để đó dùng sau, không cần cấu hình bây giờ).

Việc còn lại = tạo project + deploy + lấy 5 giá trị token/ID điền vào Script Properties.

---

## PHẦN A — Tạo & deploy Apps Script (Claude làm cùng anh qua trình duyệt được)

1. Đăng nhập **đúng Gmail cá nhân mới** của kênh Công Nghệ Số trên Chrome (KHÔNG Workspace).
2. `script.google.com` → **Dự án mới**.
3. Xoá code mẫu → dán toàn bộ `Code.gs` → Ctrl+S, đặt tên "CNS News Fetch".
...
3. **Tài khoản → Trang** → chọn Fanpage Công Nghệ Số → **Thêm người** → chọn System User vừa tạo
...
1. `console.cloud.google.com` — đăng nhập bằng **tài khoản Google sở hữu kênh YouTube Công Nghệ Số**.
...
"−" cạnh dòng — kiểm tra lại đủ 5 dòng sau khi lưu.

## PHẦN D — Sau khi có đủ 5 property
...
2. Test đăng thật (Claude chạy giúp qua `curl`, cần 1 URL mp4 + jpg công khai — vd. push video
   `astra-video` lên GitHub repo, hoặc host tạm):
   - Facebook Reel: `POST <exec> {"action":"publish_facebook","video_url":"<mp4>","caption":"...","thumbnail_url":"<jpg>","video":"astra-test","title":"..."}`
   - YouTube: `POST <exec> {"action":"publish_youtube","video_url":"<mp4>","title":"... #Shorts","description":"...","privacy":"public","thumbnail_url":"<jpg>","video":"astra-test"}`
