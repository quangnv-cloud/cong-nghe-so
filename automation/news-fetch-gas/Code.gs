...
var NEWS_IMAGE_FOLDER_NAME = 'CNS_NEWS_IMAGES'; // 
...
function getSheet_() {
  ...
  if (ssId) {
    ...
    ss = SpreadsheetApp.create('CNS News Queue');
...
function getPostsSheet_() {
  var props = PropertiesService.getScriptProperties();
  var ssId = props.getProperty('SPREADSHEET_ID');
  var ss = ssId ? SpreadsheetApp.openById(ssId) : SpreadsheetApp.create('CNS News Queue');
